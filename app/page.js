...
export const metadata = { title: 'MACH Digital Solutions | Growth Marketing for Service Businesses', description: 'A digital marketing agency for service-based businesses. Websites, SEO, paid media, and content — engineered as one growth system.' }
    ...
    <div style={{ background: T.bg, color: T.fg, minHeight: '100vh', fontFamily: 'Geist, system-ui, sans-serif' }}>
      ...
      <section style={{ position: 'relative', padding: 'clamp(60px, 10vw, 120px) clamp(16px, 4vw, 32px) clamp(80px, 12vw, 140px)', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 'min(1280px, 100%)', margin: '0 auto', position: 'relative', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 80, padding: '0 8px' }}>
            <a href="/contact" style={{ background: T.fg, color: T.bg, padding: 'clamp(12px, 2vw, 14px) clamp(18px, 3vw, 26px)', borderRadius: 10, fontSize: 15, fontWeight: 600, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 8, boxShadow: `0 0 30px ${T.glowPurple}` }}>Get started <span>→</span></a>
            <a href="/services" style={{ background: 'rgba(255,255,255,0.05)', color: T.fg, padding: 'clamp(12px, 2vw, 14px) clamp(18px, 3vw, 26px)', borderRadius: 10, fontSize: 15, fontWeight: 500, textDecoration: 'none', border: `1px solid ${T.border}`, backdropFilter: 'blur(10px)' }}>See how it works</a>
          </div>
          {/* Product mockup */}
          <BrainMockup />
        </div>
        ...
        <div style={{ maxWidth: 'min(1280px, 100%)', margin: '0 auto', position: 'relative' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(280px, 100%), 1fr))', gap: 16 }}>
            {content.services.map(s => (
              <a key={s.tag} href="/services" style={{ display: 'block', padding: 28, background: T.panel, border: `1px solid ${T.border}`, borderRadius: 16, textDecoration: 'none', color: 'inherit', position: 'relative', overflow: 'hidden' }}>
      ...
      <section style={{ padding: 'clamp(80px, 12vw, 140px) clamp(16px, 4vw, 32px)' }}>
        <div style={{ maxWidth: 'min(1280px, 100%)', margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'end', marginBottom: 48, flexWrap: 'wrap', gap: 24 }}>
            ...
            <a href="/industries" style={{ color: T.accent2, fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>All industries →</a>
          </div>
      ...
      <section style={{ padding: 'clamp(80px, 14vw, 160px) clamp(16px, 4vw, 32px)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 'min(900px, 100%)', margin: '0 auto', textAlign: 'center', position: 'relative' }}>
          ...
          <a href="/contact" style={{ background: T.fg, color: T.bg, padding: '16px 32px', borderRadius: 10, fontSize: 16, fontWeight: 600, textDecoration: 'none', display: 'inline-block', boxShadow: `0 0 40px ${T.glowPurple}` }}>Start a conversation →</a>
        </div>
      </section>
