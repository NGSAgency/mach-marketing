              ...
              <div key={cat} style={{ marginBottom: 48 }}>
                ...
                <div style={{ display: 'grid', gap: 10 }}>
                  {c.services.filter(s => s.category === cat).map(svc => (
                    <a key={svc.slug} href={`${base}/${svc.slug}-in-${slugify(area)}`} style={{ textDecoration: 'none', background: T.colors.bgLight, border: `1px solid ${T.colors.border}`, padding: '18px 24px', borderRadius: T.radius.sm, display: 'flex', alignItems: 'center', gap: 16 }}>
