    ...
    <div style={{ background: T.bg, color: T.fg, minHeight: '100vh', fontFamily: 'Manrope, system-ui, sans-serif' }}>
      ...
      <section style={{ position: 'relative', padding: 'clamp(80px, 12vw, 160px) clamp(16px, 4vw, 32px) clamp(60px, 10vw, 120px)', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 'min(1280px, 100%)', margin: '0 auto', position: 'relative' }}>
          <h1 style={{ fontFamily: 'Instrument Serif, Georgia, serif', fontSize: 'clamp(44px, 8.5vw, 108px)', fontWeight: 400, letterSpacing: -3, lineHeight: 0.98, margin: '0 0 32px 0', maxWidth: 1100 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(200px, 100%), 1fr))', gap: 32, paddingTop: 48, borderTop: `1px solid ${T.border}` }}>
            {[
              ...
              <div key={s.l}>
                <div style={{ fontFamily: 'Instrument Serif, Georgia, serif', fontSize: 'clamp(32px, 4vw, 44px)', color: T.accent, fontWeight: 500, lineHeight: 1, letterSpacing: -1, marginBottom: 8 }}>{s.n}</div>
      ...
      <section style={{ padding: 'clamp(80px, 12vw, 140px) clamp(16px, 4vw, 32px)', background: T.bgAlt, borderTop: `1px solid ${T.border}` }}>
        <div style={{ maxWidth: 'min(1280px, 100%)', margin: '0 auto' }}>
          <div style={{ marginBottom: 64, maxWidth: 900 }}>
            ...
            <h2 style={{ fontFamily: 'Instrument Serif, Georgia, serif', fontSize: 'clamp(32px, 6vw, 60px)', fontWeight: 400, letterSpacing: -2, lineHeight: 1.05, margin: '0 0 24px 0' }}>
          ...
          <div className="sentry-services-grid" style={{ display: 'grid', gap: 20 }}>
            {SERVICES.map(s => (
              <a key={s.tag} href={`/mockups/sentry/services/${s.slug}`} style={{ display: 'block', padding: 32, background: T.bgLift, border: `1px solid ${T.border}`, borderRadius: 12, textDecoration: 'none', color: 'inherit', position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ fontFamily: 'Instrument Serif, Georgia, serif', fontSize: 13, color: T.accent, fontWeight: 500, marginBottom: 20, fontStyle: 'italic' }}>{s.tag}</div>
                <div style={{ fontFamily: 'Instrument Serif, Georgia, serif', fontSize: 24, fontWeight: 500, letterSpacing: -0.5, marginBottom: 12 }}>{s.name}</div>
          ...
          <div className="sentry-industries-grid" style={{ display: 'grid', gap: 16 }}>
            {INDUSTRIES.map((i, idx) => (
              <div key={i.name} style={{ padding: 'clamp(28px, 3vw, 40px)', background: T.bgAlt, border: `1px solid ${T.border}`, borderRadius: 12, position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ fontFamily: 'Instrument Serif, Georgia, serif', fontStyle: 'italic', fontSize: 13, color: T.accent, marginBottom: 16, fontWeight: 500 }}>0{idx + 1}</div>
                <div style={{ fontFamily: 'Instrument Serif, Georgia, serif', fontSize: 22, fontWeight: 500, letterSpacing: -0.5, marginBottom: 10 }}>{i.name}</div>
      ...
      <section style={{ padding: 'clamp(80px, 12vw, 140px) clamp(16px, 4vw, 32px)', background: T.bgAlt, borderTop: `1px solid ${T.border}` }}>
        <div style={{ maxWidth: 'min(1100px, 100%)', margin: '0 auto' }}>
          ...
          <blockquote style={{ fontFamily: 'Instrument Serif, Georgia, serif', fontSize: 'clamp(28px, 5vw, 52px)', fontWeight: 400, letterSpacing: -1.5, lineHeight: 1.25, margin: 0 }}>
      ...
      <section style={{ padding: 'clamp(80px, 12vw, 140px) clamp(16px, 4vw, 32px)' }}>
        <div style={{ maxWidth: 'min(1100px, 100%)', margin: '0 auto', padding: 'clamp(48px, 8vw, 96px) clamp(24px, 5vw, 64px)', borderRadius: 16, background: `linear-gradient(135deg, ${T.bgLift}, ${T.bgAlt})`, border: `1px solid ${T.border}`, position: 'relative', overflow: 'hidden', textAlign: 'center' }}>
          ...
          <div style={{ position: 'relative' }}>
            <h2 style={{ fontFamily: 'Instrument Serif, Georgia, serif', fontSize: 'clamp(32px, 6vw, 64px)', fontWeight: 400, letterSpacing: -2, lineHeight: 1.05, margin: '0 0 24px 0' }}>Schedule a <em style={{ fontStyle: 'italic', color: T.accent }}>complimentary</em> strategy call.</h2>
