    ...
    <div style={{ background: T.bg, color: T.fg, minHeight: '100vh', fontFamily: 'Manrope, system-ui, sans-serif' }}>
      ...
      {SERVICES.map((s, idx) => (
        <a href={`/mockups/sentry/services/${s.slug}`} key={s.tag} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}><section style={{ padding: 'clamp(80px, 10vw, 120px) clamp(16px, 4vw, 32px)', borderTop: `1px solid ${T.border}`, background: idx % 2 === 1 ? T.bgAlt : T.bg }}>
          <div style={{ maxWidth: 'min(1200px, 100%)', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(280px, 100%), 1fr))', gap: 'clamp(32px, 5vw, 80px)', alignItems: 'start' }}>
            <div>
              <div style={{ fontFamily: 'Instrument Serif, Georgia, serif', fontStyle: 'italic', fontSize: 'clamp(64px, 12vw, 120px)', color: T.accent, fontWeight: 400, letterSpacing: -3, lineHeight: 0.9 }}>{s.tag}</div>
              <div style={{ fontSize: 12, color: T.fgMuted, letterSpacing: 2, textTransform: 'uppercase', fontWeight: 700, marginTop: 12 }}>{s.name}</div>
            </div>
            <div>
              <h2 style={{ fontFamily: 'Instrument Serif, Georgia, serif', fontSize: 'clamp(28px, 4.5vw, 44px)', fontWeight: 400, letterSpacing: -1.5, lineHeight: 1.1, margin: '0 0 24px 0' }}>{s.short}</h2>
      ...
      <section style={{ padding: 'clamp(80px, 12vw, 140px) clamp(16px, 4vw, 32px)', background: T.bgAlt, borderTop: `1px solid ${T.border}`, textAlign: 'center' }}>
        <div style={{ maxWidth: 'min(900px, 100%)', margin: '0 auto' }}>
          <h2 style={{ fontFamily: 'Instrument Serif, Georgia, serif', fontSize: 'clamp(32px, 6vw, 56px)', fontWeight: 400, letterSpacing: -2, lineHeight: 1.05, margin: '0 0 32px 0' }}>Let's <em style={{ fontStyle: 'italic', color: T.accent }}>build</em> together.</h2>
