...
export const metadata = { title: 'Financial Operations - Sentry Solutions', robots: { index: false, follow: false } }

export default function ServicePage() {
  const service = SERVICES.find(s => s.slug === 'financial-operations')
  if (!service) notFound()

  const otherServices = SERVICES.filter(s => s.slug !== 'financial-operations')
    ...
    <div style={{ background: T.bg, color: T.fg, minHeight: '100vh', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <section style={{ position: 'relative', padding: 'clamp(80px, 12vw, 140px) clamp(16px, 4vw, 32px) clamp(48px, 8vw, 80px)', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 'min(1200px, 100%)', margin: '0 auto', position: 'relative' }}>
          ...
          <div style={{ fontFamily: 'Fraunces, Georgia, serif', fontStyle: 'italic', fontSize: 'clamp(24px, 3vw, 36px)', color: T.accent, marginBottom: 24 }}>{service.tag}</div>
          ...
          <p style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: 'clamp(19px, 2.4vw, 28px)', color: T.fgDim, lineHeight: 1.5, margin: 0, maxWidth: 780, fontStyle: 'italic' }}>{service.short}</p>
        </div>
      </section>
      ...
      <section style={{ padding: 'clamp(60px, 10vw, 120px) clamp(16px, 4vw, 32px)', borderTop: `1px solid ${T.border}` }}>
        <div style={{ maxWidth: 'min(900px, 100%)', margin: '0 auto' }}>
          ...
          <p style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: 'clamp(18px, 2vw, 22px)', color: T.fgDim, lineHeight: 1.7, margin: 0 }}>{service.body}</p>
        </div>
      </section>
      ...
      <section style={{ padding: 'clamp(60px, 10vw, 120px) clamp(16px, 4vw, 32px)', background: T.bgAlt, borderTop: `1px solid ${T.border}` }}>
        <div style={{ maxWidth: 'min(1200px, 100%)', margin: '0 auto' }}>
          <div style={{ marginBottom: 48, maxWidth: 800 }}>
            ...
            <h2 style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: 'clamp(28px, 5vw, 48px)', fontWeight: 400, letterSpacing: -1.5, lineHeight: 1.1, margin: 0 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(260px, 100%), 1fr))', gap: 16 }}>
            {[
              ...
              <div key={i} style={{ padding: 28, background: T.bg, border: `1px solid ${T.border}`, borderRadius: 12 }}>
                <div style={{ fontFamily: 'Fraunces, Georgia, serif', fontStyle: 'italic', fontSize: 24, color: T.accent, marginBottom: 12 }}>0{i + 1}</div>
                <div style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: 20, fontWeight: 500, letterSpacing: -0.5, marginBottom: 10 }}>{item.t}</div>
      ...
      <section style={{ padding: 'clamp(60px, 10vw, 120px) clamp(16px, 4vw, 32px)', borderTop: `1px solid ${T.border}` }}>
        <div style={{ maxWidth: 'min(900px, 100%)', margin: '0 auto', textAlign: 'center' }}>
          <h2 style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: 'clamp(28px, 5vw, 48px)', fontWeight: 400, letterSpacing: -1.5, lineHeight: 1.1, margin: '0 0 24px 0' }}>Explore how {service.name.toLowerCase()} can help.</h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(240px, 100%), 1fr))', gap: 12 }}>
            {otherServices.map(s => (
              <a key={s.tag} href={`/mockups/sentry/services/${s.slug}`} style={{ padding: 20, background: T.bg, border: `1px solid ${T.border}`, borderRadius: 8, textDecoration: 'none', color: 'inherit', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
                <div>
                  <div style={{ fontFamily: 'Fraunces, Georgia, serif', fontStyle: 'italic', fontSize: 12, color: T.accent, marginBottom: 4 }}>{s.tag}</div>
                  <div style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: 16, fontWeight: 500 }}>{s.name}</div>
                </div>
