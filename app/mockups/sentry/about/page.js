    ...
    <div style={{ background: T.bg, color: T.fg, minHeight: '100vh', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <section style={{ position: 'relative', padding: 'clamp(80px, 12vw, 140px) clamp(16px, 4vw, 32px) clamp(48px, 8vw, 80px)', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 'min(1200px, 100%)', margin: '0 auto', position: 'relative' }}>
          ...
          <h1 style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: 'clamp(44px, 8vw, 96px)', fontWeight: 400, letterSpacing: -3, lineHeight: 0.98, margin: '0 0 40px 0' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(300px, 100%), 1fr))', gap: 'clamp(32px, 5vw, 64px)' }}>
            <p style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: 'clamp(19px, 2.2vw, 24px)', color: T.fgDim, lineHeight: 1.6, margin: 0 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(260px, 100%), 1fr))', gap: 24 }}>
            {[
              ...
              <div key={i} style={{ padding: 32, background: T.bgLift, border: `1px solid ${T.border}`, borderRadius: 12 }}>
                <div style={{ fontFamily: 'Fraunces, Georgia, serif', fontStyle: 'italic', fontSize: 32, color: T.accent, fontWeight: 500, marginBottom: 16 }}>0{i + 1}</div>
                <div style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: 22, fontWeight: 500, letterSpacing: -0.5, marginBottom: 12 }}>{v.t}</div>
        ...
        <div style={{ maxWidth: 'min(1200px, 100%)', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(220px, 100%), 1fr))', gap: 32, borderTop: `1px solid ${T.borderStrong}`, paddingTop: 40 }}>
            {[
              ...
              <div key={i}>
                <div style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: 'clamp(56px, 8vw, 88px)', color: T.accent, fontWeight: 400, lineHeight: 0.9, letterSpacing: -3, marginBottom: 16 }}>{s.n}</div>
      ...
      <section style={{ padding: 'clamp(80px, 12vw, 140px) clamp(16px, 4vw, 32px)', background: T.bgAlt, borderTop: `1px solid ${T.border}`, textAlign: 'center' }}>
        <div style={{ maxWidth: 'min(900px, 100%)', margin: '0 auto' }}>
          <h2 style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: 'clamp(32px, 6vw, 56px)', fontWeight: 400, letterSpacing: -2, lineHeight: 1.05, margin: '0 0 32px 0' }}>Let's <em style={{ fontStyle: 'italic', color: T.accent }}>talk</em>.</h2>
