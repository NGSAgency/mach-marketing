...
export function renderEmail(opts) {
  ...
  if (details && details.length > 0) {
    const rows = details.map(([label, value], i) => {
      ...
      return `<tr><td style="padding: 12px 16px 12px 0; color: #666; font-size: 13px; font-weight: 500; vertical-align: top; width: 140px; ${border}">${label}</td><td style="padding: 12px 0; font-weight: 700; color: #050508; font-size: 14px; vertical-align: top; ${border}">${value}</td></tr>`
    }).join('')
    detailsHtml = `<table role="presentation" cellspacing="0" cellpadding="0" border="0" style="width: 100%; background: #f5f5f7; border-radius: 12px; padding: 12px 24px; margin: 24px 0; border-collapse: separate; box-sizing: border-box;">${rows}</table>`
  }
