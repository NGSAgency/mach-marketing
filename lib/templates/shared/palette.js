 ...
 */
function pushUntil(color, toward, against, target) {
 ...
 */
function shiftLightnessUntil(color, darker, against, target) {
...
export function buildPalette({ brandColor, mode = 'light', secondary = null }) {
  ...
  // Backgrounds first; everything else is checked against them.
  const bg = dark ? mix('#0d0d0f', brand, 0.05) : mix('#ffffff', brand, 0.02)
  const bgAlt = dark ? mix(bg, brand, 0.05) : mix('#ffffff', brand, 0.06)
  const bgRaised = dark ? mix(bg, '#ffffff', 0.06) : '#ffffff'
  const surface = dark ? mix(bg, '#ffffff', 0.04) : '#ffffff'
  const surfaceAlt = dark ? mix(bg, brand, 0.09) : mix('#ffffff', brand, 0.09)
  ...
  const accent = shiftLightnessUntil(brand, !dark, [...grounds, accentGlow], TARGETS.accent)
  const onAccent = contrast('#ffffff', accent) >= contrast('#141416', accent) ? '#ffffff' : '#141416'
