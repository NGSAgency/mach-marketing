...
import { buildPalette, pickBrandColor, contrast } from './palette.js'
...
export function applyBrand(baseTokens, brand = {}) {
  ...
  // kept wherever it passes 4.5:1; a brand it fails on (white on yellow) gets
  // whichever of white or near-black reads better, the same rule buildPalette
  // uses. Sites whose buttons already pass are unchanged.
  if (/^#[0-9a-f]{6}$/i.test(accent) && contrast(base.onAccent, accent) < 4.5) {
