...
import { hasFaqPage } from '../claims.js'
...
export function generateSitemap(config, baseUrl, posts = []) {
  ...
  const urls = [
    ...
    // Only when there are questions on it: an empty FAQ page is a thin page.
    ...(hasFaqPage(config) ? [{ url: url('/faq'), lastModified: now, priority: 0.7, changeFrequency: 'monthly' }] : []),
    // Low priority, but it should be findable rather than orphaned.
    { url: url('/privacy'), lastModified: now, priority: 0.2, changeFrequency: 'yearly' },
  ]
