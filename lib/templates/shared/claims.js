...
/**
 * A service or area name set mid-sentence. Capitals the client wrote are
 * theirs: "AC Repair" belongs in a heading as "AC repair", not "ac repair",
 * and "HVAC" never becomes "hvac". Only ordinary capitalised words are
 * lowered, so a name that is already lower case comes back unchanged.
 */
export function lowerName(value) {
  return String(value || '')
    .split(' ')
    .map(w => {
      const letters = w.replace(/[^A-Za-z]/g, '')
      // Two or more letters, all of them capitals: an initialism, left alone.
      return letters.length > 1 && letters === letters.toUpperCase() ? w : w.toLowerCase()
    })
    .join(' ')
}
...
export function whyUsItems(c, service) {
  ...
  // Certifications were collected on the questionnaire, offered in the
  // section registry, and rendered by no family at all. They belong with the
  // licence and the warranty: a trade certification is the same kind of claim
  // and the same kind of reassurance. Two at most, because this list sits
  // beside a button and is read at a glance.
  const certs = (c?.certifications || [])
    .map(x => (typeof x === 'string' ? x : x?.name))
    .filter(x => typeof x === 'string' && x.trim())
  for (const cert of certs.slice(0, 2)) items.push(cert.trim())
...
/**
 * Whether the site has an FAQ page at all.
 *
 * A site with no questions used to get one anyway: a heading and then the
 * footer, marked for indexing, listed in the sitemap, linked from SERENE's
 * menu four times, and — once the generated meta reached it — titled with a
 * promise of answers it did not have. An empty page is a thin page. The page,
 * the sitemap and every menu ask this, so they cannot disagree. A concept
 * keeps its page, which shows what the FAQ will hold.
 */
export const hasFaqPage = (c) => c?.concept === true || faqsFrom(c).length > 0
