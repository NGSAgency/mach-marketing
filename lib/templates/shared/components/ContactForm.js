...
import { trackEvent } from '../../../site/events.js'
 ...
 */
export default function ContactForm({ slug, concept = false, colors, fonts = {}, radius = 6, submitLabel = 'Send', rows = 5 }) {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      // A lead only counts once it has actually reached the client.
      trackEvent('form_submit', { method: 'form' })
      setState('sent')
    } catch (err) {
  ...
  return (
    <form onSubmit={onSubmit} style={{ display: 'grid', gap: 18, fontFamily: fonts.body }}>
      ...
      <div>
        ...
        <textarea id="cf-message" name="message" rows={rows} required style={{ ...field, resize: 'vertical' }} />
      </div>
