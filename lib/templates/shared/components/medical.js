...
export function TrustBar({ T, c }) {
  ...
  } else if (providers.length > 0 && providers[0].title) {
    // "1 licensed provider" reads as a weakness. Naming the credential is the
    // actual signal.
    items.push({ big: providers[0].name.split(' ').slice(-1)[0], small: providers[0].title.split(',')[0] })
  }
