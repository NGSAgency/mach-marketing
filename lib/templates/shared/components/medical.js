...
export function BeforeAfterGallery({ T, c, cases = [] }) {
  if (cases.length === 0) {
    return (
      <section style={{ background: T.colors.bgAlt, padding: 'clamp(56px, 8vw, 104px) 32px' }}>
        <div style={{ maxWidth: 720, margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 16, color: T.colors.textDim, lineHeight: 1.7, margin: 0 }}>
            A filterable gallery of your own before and after results goes here. This is
            consistently the strongest converting section on a practice website, and it only
            works with genuine photography and documented client consent.
          </p>
        </div>
      </section>
    )
  ...
  return (
    <section style={{ background: T.colors.bgAlt, padding: 'clamp(56px, 8vw, 104px) 32px' }}>
      <div style={{ maxWidth: 'min(1100px, 100%)', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(300px, 100%), 1fr))', gap: 24 }}>
          {cases.map((item, i) => (
            <figure key={i} style={{ margin: 0 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2, borderRadius: T.radius.md, overflow: 'hidden' }}>
              ...
              <figcaption style={{ fontSize: 13, color: T.colors.textDim, marginTop: 10 }}>
                {item.treatment}
        ...
        <p style={{ fontSize: 12, color: T.colors.textMuted, marginTop: 28 }}>
