...
// Set by the middleware when the page is served on a client's own domain.
async function siteOrigin() {
  try {
    const v = (await headers()).get('x-mach-site-origin')
    return v && /^https:\/\/[a-z0-9.-]+$/i.test(v) ? v : null
  } catch { return null }
}

// Set on MACH staff browsers (middleware markStaff); passed on so Command
// Center doesn't count our own views of concepts and previews.
async function staffViewer() {
  try { return (await cookies()).get('mach_staff')?.value || null } catch { return null }
}
...
export async function fetchSiteConfig({ slug, clientId, domain, page, id }) {
  ...
  const result = await fetchSiteConfigCached(params.toString())
  // On the client's own domain the middleware says so, and every link on the
  // page goes to its clean address there (https://kcskin.com/treatments/botox)
  // rather than under /site/<slug>.
  const origin = await siteOrigin()
  return origin && result?.config ? { ...result, config: { ...result.config, base_path: origin } } : result
}
...
export async function fetchPreviewConfig(token) {
  try {
    const viewer = await staffViewer()
    const res = await fetch(`${PREVIEW_API}?token=${encodeURIComponent(token)}${viewer ? `&viewer=${encodeURIComponent(viewer)}` : ''}`, { cache: 'no-store' })
    if (!res.ok) return null
    return withStock(await res.json())
  } catch (e) { return null }
}
...
export const fetchMockup = cache(async (token) => {
  try {
    const viewer = await staffViewer()
    const res = await fetch(`${MOCKUP_API}?token=${encodeURIComponent(token)}${viewer ? `&viewer=${encodeURIComponent(viewer)}` : ''}`, { cache: 'no-store' })
